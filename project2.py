...
class clothPoint:
    def __init__(self,row,col):
        ...
        self.xacc = 0 # accelerations defined later
        self.yacc = 0
...
for i in range(N):
    for j in range(N):
        pointlist[i][j].xpos[0] = 0 # initial x position
        pointlist[i][j].ypos[0] = 0 # initial y position
...
fig = plt.figure()
ax = fig.add_subplot(111, aspect='equal', xlim=(-1, N),ylim=(-1, N))
ax.set_xlabel('X')
ax.set_ylabel('Y')
